...
function normalizePackageKeys(keys) {
...
async function touchPlaceFile(options = {}) {
  ...
  const packageKeys = normalizePackageKeys(options.packageKeys);

  if (options.packageSourcePlaceId && packageKeys.length > 0) {
    args.push("--package-source-place-id", String(options.packageSourcePlaceId));
  }

  if (packageKeys.length > 0) {
    args.push("--update-package-keys", JSON.stringify(packageKeys));
  }
...
async function indexPlacePackages(options = {}) {
  const script = getTouchScriptPath();

  if (!options.apiKey) {
    throw new Error("An API key is required for package indexing.");
  }

  if (!options.placeId) {
    throw new Error("A Place ID is required for package indexing.");
  }
...
async function verifyPlaceMarker(options = {}) {
  const script = getTouchScriptPath();

  if (!options.apiKey) {
    throw new Error("An API key is required for place verification.");
  }

  if (!options.placeId) {
    throw new Error("A Place ID is required for place verification.");
  }
  ...
  if (options.expectedValue) {
    args.push("--expected-value", options.expectedValue);
  }
